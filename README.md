...
- `XQQI` / `QQQI` 배당 권리 최종 매수 가능일 계산, 텔레그램 알림, 10분 주기 조회 헬스체크 운영 알림
...
운영 기준 현재 구조는 아래와 같습니다.

- 백엔드: `blue`/`green` dual-slot, 내부 안정 주소 `http://127.0.0.1:9000`
- 프런트엔드: `blue`/`green` dual-slot, 내부 안정 주소 `http://127.0.0.1:3100`
