...
- `POST /api/v1/jobs/distribution-deadline-preview`
...
`POST /api/v1/jobs/distribution-deadline-check` 는 NEOS 공식 `XQQI` / `QQQI` 분배 일정을 읽고, 배당 권리를 받기 위한 마지막 매수 가능 미국 정규장 세션을 계산하고 발송 상태를 갱신합니다.

`POST /api/v1/jobs/distribution-deadline-preview` 는 같은 계산을 read-only 로 수행합니다. 응답 구조는 동일하지만 `distribution_deadline_states`, `distribution_deadline_alerts` 를 변경하지 않습니다.
...
운영에서는 `distribution_deadline_states.last_alert_key` 로 중복 발송을 막고, `distribution_deadline_alerts` 에 발송 이력을 남깁니다. 이 상태 변경은 `distribution-deadline-check` 에서만 수행됩니다.

배당 알림 workflow 와 별도 health workflow 는 계산 로직은 공유하지만, 서로 다른 endpoint 를 사용합니다.

- `neos-distribution-deadline.json`: `05:30 KST` 배당 알림 전용
- `neos-distribution-health.json`: `10분`마다 `distribution-deadline-preview` 를 호출해 endpoint 장애를 감시
