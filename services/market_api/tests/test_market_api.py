...
class MarketApiTests(unittest.TestCase):
    ...
    @patch.object(deps.settings, "job_shared_secret", "test-job-secret")
    @patch(
        "services.market_api.app.routers.market.market_service.run_distribution_deadline_preview",
        return_value=schemas.DistributionDeadlineCheckResponse(
    ...
    def test_distribution_deadline_preview_returns_structured_payload(
        self,
        _run_distribution_deadline_preview: object,
    ) -> None:
        with self.SessionLocal() as db:
            response = market.run_distribution_deadline_preview("test-job-secret", db)
